...
const rtsMessageSchema = z.looseObject({
...
export class Slack {
  ...
  async search(query: string, limit = 5): Promise<Result<SlackRtsResult[]>> {
    try {
      ...
      const results = this.normaliseResponse(response);
  ...
  /** Normalise Slack RTS responses into the compact shape used by the agent. */
  public normaliseResponse(response: unknown): SlackRtsResult[] {
