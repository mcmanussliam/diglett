import AnthropicClient, { type ClientOptions } from "@anthropic-ai/sdk";
...
export class Anthropic {
  private readonly logger = log.child({ name: Anthropic.name });

  private readonly client: AnthropicClient;

  constructor(options: ClientOptions = {}) {
    this.client = new AnthropicClient({ apiKey: env.ANTHROPIC_API_KEY, ...options });
  }

  public async chat(messages: MessageParam[], tools?: Tool[]): Promise<Result<Message>> {
  ...
  public parse(text: string): Result<Diagnosis> {
